...
profile <- profile %>%
  ...
  mutate(abundance_z = as.numeric(scale(abundance))) %>%
  ungroup() %>%
  filter(is.finite(signed_distance), is.finite(abundance_z))

prediction_grid <- profile %>%
  distinct(sample, cell_state) %>%
  tidyr::crossing(signed_distance = seq(-6, 6, by = 0.1))

fit_one <- function(sample_name, state_name) {
  subset <- profile %>%
    filter(sample == sample_name, cell_state == state_name)
  model <- mgcv::gam(
    abundance_z ~ s(signed_distance, k = 5),
    data = subset,
    method = "REML"
  )
...
write_run_metadata(
  ...
  list(
    ...
    signed_distance_range = "-6 to 6 spot rings"
  )
)
